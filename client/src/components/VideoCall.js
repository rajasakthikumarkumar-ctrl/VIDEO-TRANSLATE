...
function VideoCall() {
  ...
  const [recordingType, setRecordingType] = useState('both');
  const [recordedChunks, setRecordedChunks] = useState([]);
  const [recordings, setRecordings] = useState([]);
  const [showRecordings, setShowRecordings] = useState(false);

  // Meeting history — persisted in localStorage
  const [meetingHistory, setMeetingHistory] = useState(() => {
    try { return JSON.parse(localStorage.getItem('meetingHistory') || '[]'); } catch { return []; }
  });
  ...
  const autoRecordingRef = useRef(null); // ref to the auto-started MediaRecorder
